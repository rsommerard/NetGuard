...
void check_icmp_socket(const struct arguments *args, const struct epoll_event *ev) {
    ...
    if (ev->events & EPOLLERR) {
        ...
        if (err < 0) {
            log_android(ANDROID_LOG_ERROR, "ICMP getsockopt error %d: %s", errno, strerror(errno));
        } else if (serr) {
...
jboolean handle_icmp(const struct arguments *args,
    ...
    if (sendto(cur->socket, icmp, (socklen_t) icmplen, MSG_NOSIGNAL, (const struct sockaddr *) (version == 4 ? &server4 : &server6), (socklen_t) (version == 4 ? sizeof(server4) : sizeof(server6))) != icmplen) {
